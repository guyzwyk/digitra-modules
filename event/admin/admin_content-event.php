<?php
	require("../modules/event/langfiles/".$langFile.".php"); //Module language pack
...
<!-- ********************************* Create event cat  ******************************************************
      ...
      ****************************************************************************************************************-->
      ...
      <?php if($_REQUEST['what'] == "catUpdate") {//if(isset($rub_displayUpd)){ ?>
      ...
      ****************************************************************************************************************-->
      <?php if($_REQUEST[what]=="insert") { ?>
      		<?php 
      			...
      			//Si 01 langue, attribuer la langue systeme
      			$currentLang = (($_REQUEST[langId] != '') ? ($_REQUEST[langId]) : ($_SESSION['LANG']));
      		?>
	            	...
	            	<div class="form-group">
	                    ...
	                    <div class="col-sm-8">
	                       	<select class="form-control"  name="sel_eventLang" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)">
								<?php print stripslashes($event->redir_cmb_load_langs($_REQUEST['langId'], $event->admin_modPage, "insert")); ?>
	                          </select>
	                          <?php //$settings = $system->get_site_settings(); deja defini dans admin.php ?>
	          				...
	          				<div class="panel-body no-padding">
			      				<?php
			      					/*:::::: Calendar settings :::::*/
			      						...
			      						$myCal->link				=	$event->admin_modPage.'&what=insert'; //$thewu32_site_url.'/modern_admin/'.$event->admin_modPage.'&what=insert';
			      					...
			      					if(isset($_POST[btn_calDisplay])){
			      						...
			      						if(is_array($arrEvents)){
				      						foreach($arrEvents as $calEvent){
				      						    $myCal->viewEvent($calEvent['DAY-START'], $calEvent['DAY-END'], '#ccc', $calEvent['TITLE'], $myCal->link);
				      						}
			      						} 
									  }
									  
									$arrEvents	=	$event->calendar_get_events($event->get_datetime());
			      					if(is_array($arrEvents)){
				      					foreach($arrEvents as $calEvent){
				      					    $myCal->viewEvent($calEvent['DAY-START'], $calEvent['DAY-END'], '#ccc', $calEvent['TITLE'], $myCal->link);
				      					}
	...
	<!-- Tabs contents end -->        
      ...
      ****************************************************************************************************************-->
      <?php 
      	if($_REQUEST[what] == "update") { // Si on a clique sur modifier l'event... 
