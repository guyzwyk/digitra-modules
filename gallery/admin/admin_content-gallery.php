...
<!-- Modifier les images -->
<a name="UPDATE"></a>
      <?php if(($_REQUEST[what] == "update") && ($_REQUEST[action]=="update")){ $tab_gallery = $gallery->get_gallery($_REQUEST[$gallery->URI_galleryVar]);?>
     ...
     <?php $currentLang = (($_REQUEST[langId] != '') ? ($_REQUEST[langId]) : ($_SESSION['LANG'])); ?>
	  ...
	  <!-- Afficher les images -->
	  <?php if(($_REQUEST[what]=="display") || ($_REQUEST[limite] != '')){ ?>
	  ...
	  <?php if(($_REQUEST[what]=="displayByCat")){ ?>
      ...
      <!-- Modifier les albums -->
      <?php if($_REQUEST[action]=="catUpdate"){ ?>
      	<div class="col-sm-6">
	      <h3><?php print $mod_lang_output['MODULE_NAME']; ?> :: <?php print $mod_lang_output['PAGE_HEADER_UPDATE_GALLERY_CATEGORY']; ?></h3>
    ...
    <?php if($_REQUEST[what]=="commentDisplay") { ?>
