<?php
	class yafe_wishes extends cwd_system{
		var $tbl_wishes;
		var $fld_wishesId;
		var $fld_wishesDate;
		...
		function update_wishes($new_wishesId,
							   $new_wishesName,
							   $new_wishesEmail,
							   $new_wishesContent,
							   $new_wishesDate){
			...
			if($total = mysqli_num_rows($result)){
				...
				while($row = mysqli_fetch_array($result)){
					...
					$toRet .= "<div class=\"react_container\">
								  ...
								  <div class=\"react_container_body\">".nl2br($wishes_content)."</div>
							   </div>";
				}
				$toRet .= "</div>";
			}
			else{
				$toRet = "<div class=\"react_no_comment\">Aucun v&oelig;u &agrave; afficher pour l'instant.</div>";
			}
			return $toRet;
		}
		
		function display_valid_wishes($nombre="10", $limite="0"){
			...
			return $toRet."<span class=\"name\">YA-FE : </span><apn><a href=\"wishes.php\">Diffusez vos messages en cliquant ici</a></span>";
		}
	}
?>